...
class FirebaseService {
  ...
  static Future<Event> getEventById(String eventId) async {
    _logger.info('Starting getEventById operation for eventId: $eventId');
    return await _performDatabaseOperation<Event>(
      () async {
        final snapshot = await _eventRef.child(eventId).get();
        if (snapshot.exists) {
          _logger.info('Snapshot received for eventId: $eventId');
          ...
          if (data is List) {
            _logger.warning('Data received is a List, converting to Map.');
            eventMap = {for (int i = 0; i < data.length; i++) '$i': data[i]};
          } else if (data is Map) {
            eventMap = Map<String, dynamic>.from(data as Map);
          } else {
            _logger.warning('Unexpected data type: ${data.runtimeType}');
            throw TypeError();
          }

          eventMap['id'] = eventId; // Ensure eventMap has id
          _logger.info('Event map created: $eventMap');
              ...
              if (participantValue is Map) {
                try {
                  final participantMap =
                      Map<String, dynamic>.from(participantValue);
                  _logger.info('Participant map created: $participantMap');
                  participants.add(UserModel.fromMap(participantMap));
                } catch (e) {
                  _logger
                      .warning('Error parsing participant: $participantValue');
                }
              } else {
                _logger
                    .warning('Invalid participant format: $participantValue');
              }
            });
            eventMap['participants'] = participants;
          ...
          _logger.warning('Event not found for eventId: $eventId');
